...
> 一个素材发布 Skill 原本只适合网站。后来同一批日语学习素材要给网站、App、视频流水线共用，并区分国内 / 海外访问路径。旧 Skill 里“直接写完整 URL”或“只面向网站路径”的假设开始不适用。

不要展开所有服务器、数据库和项目细节。场景只服务于“Skill 遇到变化”这条主线。

### 5. 同一句需求，两种起手式

展示用户最容易感知的差异：

- 普通 Agent：先反问服务器、OSS、数据库、URL 策略。
- EvoMap Agent：先 recall 共享素材 Gene / Capsule，再给 `asset_manifest + object_key + AssetUrlResolver` 的最小实现方案。
